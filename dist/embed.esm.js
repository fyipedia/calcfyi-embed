...
.guidefyi-card-footer { padding: 8px 16px; border-top: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; }
  `}function ae(e){switch(e){case"clean":return z();case"modern":default:return H()}}function D(e,r="modern"){return`
:host {
  display: block;
  --site-accent: ${e};
}
...
${ae(r)}
  `}function w(e,r){let t=e.dataset.styleVariant||"modern",i=e.attachShadow({mode:"open"}),a=document.createElement("style");return a.textContent=D(r.accent,t),i.appendChild(a),i}function oe(e){let r=e.dataset.theme||"light";return r==="auto"?window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light":r}function $(e,r,t){let i=oe(r),a=r.dataset.size||"default",o=document.createElement("div");return o.className=["guidefyi-widget",t].filter(Boolean).join(" "),o.setAttribute("data-theme",i),o.setAttribute("data-size",a),e.appendChild(o),r.dataset.theme==="auto"&&window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",n=>{o.setAttribute("data-theme",n.matches?"dark":"light")}),o}function f(e){e.innerHTML=`
    <div class="guidefyi-loading">
      <span class="guidefyi-spinner"></span>
      Loading\u2026
    </div>
  `}function s(e,r,t){e.innerHTML=`
    <div class="guidefyi-error">
      <p>${r}</p>
      <a href="https://${t.domain}" target="_blank" rel="noopener">
        Visit ${t.name}
      </a>
    </div>
  `}var P='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>';function l(e){return`<span class="guidefyi-powered">Powered by <a href="https://${e.domain}" target="_blank" rel="noopener">${e.name}</a></span>`}function C(e){return`guidefyi_embed_${e}`}function ne(e){try{let r=sessionStorage.getItem(C(e));if(!r)return null;let t=JSON.parse(r);return Date.now()-t.ts>3e5?(sessionStorage.removeItem(C(e)),null):t.data}catch{return null}}function se(e,r){try{let t={data:r,ts:Date.now()};sessionStorage.setItem(C(e),JSON.stringify(t))}catch{}}async function c(e,r,t){let i=e.endsWith("/")?e:e+"/",a=r.startsWith("/")?r.slice(1):r,o=new URL(a,i);t&&Object.entries(t).forEach(([x,v])=>o.searchParams.set(x,v));let n=o.toString(),d=ne(n);if(d!==null)return d;let g=await fetch(n,{headers:{Accept:"application/json"}});if(!g.ok)throw new Error(`API error ${g.status}: ${n}`);let y=await g.json();return se(n,y),y}async function A(e,r,t){let i=await c(e,r,t);return Array.isArray(i)?i:i&&typeof i=="object"&&"results"in i?i.results:[]}var h={calculatorDetail:e=>`calculators/${e}/`,calculatorList:"calculators/",formulaDetail:e=>`formulas/${e}/`,formulaList:"formulas/",categoryList:"categories/",categoryDetail:e=>`categories/${e}/`,glossaryDetail:e=>`glossary/${e}/`,glossaryList:"glossary/",guideDetail:e=>`guides/${e}/`,guideList:"guides/",faqList:"faqs/",stats:"stats/"},L={calculator:e=>`https://calcfyi.com/calculator/${e}/`,formula:e=>`https://calcfyi.com/formula/${e}/`,category:e=>`https://calcfyi.com/category/${e}/`,glossary:e=>`https://calcfyi.com/glossary/${e}/`,guide:e=>`https://calcfyi.com/guide/${e}/`,home:"https://calcfyi.com/"};var B='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>';var q='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>',F='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/><line x1="16" y1="14" x2="16" y2="18"/><line x1="8" y1="10" x2="8" y2="10.01"/><line x1="12" y1="10" x2="12" y2="10.01"/><line x1="16" y1="10" x2="16" y2="10.01"/><line x1="8" y1="14" x2="8" y2="14.01"/><line x1="12" y1="14" x2="12" y2="14.01"/><line x1="8" y1="18" x2="8" y2="18.01"/><line x1="12" y1="18" x2="12" y2="18.01"/></svg>',I='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="3"/><circle cx="8" cy="8" r="1.5" fill="currentColor"/><circle cx="16" cy="8" r="1.5" fill="currentColor"/><circle cx="8" cy="16" r="1.5" fill="currentColor"/><circle cx="16" cy="16" r="1.5" fill="currentColor"/><circle cx="12" cy="12" r="1.5" fill="currentColor"/></svg>';function j(e){return e==null?"\u2014":e.toLocaleString("en-US")}function b(e){return e==null?"\u2014":"$"+e.toLocaleString("en-US",{maximumFractionDigits:0})}function m(e,r){return e?e.length>r?e.slice(0,r)+"\u2026":e:""}function u(e,r){return`<a class="guidefyi-link" href="${e}" target="_blank" rel="noopener">${r} ${P}</a>`}async function R(e,r,t){let i=await c(t.apiBase,h.calculatorDetail(r)),a=L.calculator(i.slug);e.innerHTML=`
    <div class="guidefyi-card-header">
      <h3 class="guidefyi-card-title">
        <a href="${a}" target="_blank" rel="noopener">${F} ${i.name}</a>
      </h3>
      <p class="guidefyi-card-subtitle">${i.category}</p>
    </div>
    <div class="guidefyi-card-body">
      ${i.description?`<p class="guidefyi-card-desc">${m(i.description,200)}</p>`:""}
      <div class="guidefyi-card-footer">
        ${u(a,"Open Calculator")}
      </div>
    </div>
    ${l(t)}
  `}async function k(e,r,t){let i=await c(t.apiBase,h.formulaDetail(r)),a=L.formula(i.slug),o=i.variables&&i.variables.length>0?`<div class="guidefyi-vars">
        ${i.variables.map(n=>`
    ...
    ${o}
    <div class="guidefyi-card-body">
      ${i.explanation?`<p class="guidefyi-card-desc">${m(i.explanation,180)}</p>`:""}
      <div class="guidefyi-card-footer">
        ${u(a,"Full Formula")}
      </div>
    </div>
    ${l(t)}
  `}function de(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}var S={countryDetail:e=>`countries/${e}/`,countryList:"countries/",cityList:"cities/",roleDetail:e=>`roles/${e}/`,roleList:"roles/",salaryData:"salary-data/",glossaryDetail:e=>`glossary/${e}/`,glossaryList:"glossary/",guideDetail:e=>`guides/${e}/`,guideList:"guides/",faqList:"faqs/",search:"search/",stats:"stats/"},T={country:e=>`https://salaryfyi.com/countries/${e}/`,role:e=>`https://salaryfyi.com/roles/${e}/`,salary:(e,r)=>`https://salaryfyi.com/salary/${e}/${r}/`,glossary:e=>`https://salaryfyi.com/glossary/${e}/`,guide:e=>`https://salaryfyi.com/guide/${e}/`,home:"https://salaryfyi.com/"};async function G(e,r,t){let i=await c(t.apiBase,S.countryDetail(r)),a=T.country(i.slug),o=n=>{if(!n)return"";let g={growing:"#16a34a",stable:"#eab308",declining:"#dc2626"}[n.toLowerCase()]||"var(--muted)";return`<span class="guidefyi-badge" style="background:${g}20;color:${g}">${n}</span>`};e.innerHTML=`
    <div class="guidefyi-card-header">
      <h3 class="guidefyi-card-title">
    ...
    <div class="guidefyi-card-body">
      <div class="guidefyi-props">
        <div class="guidefyi-prop">
          ...
          <span class="guidefyi-prop-value">${b(i.median_salary_usd)}</span>
        </div>
        <div class="guidefyi-prop">
      ...
      ${i.description?`<p class="guidefyi-card-desc">${m(i.description,160)}</p>`:""}
      <div class="guidefyi-card-footer">
        ${u(a,"Full Country Profile")}
      </div>
    </div>
    ${l(t)}
  `}async function N(e,r,t){let i=await c(t.apiBase,S.roleDetail(r)),a=T.role(i.slug),n={high:"#16a34a",growing:"#16a34a",moderate:"#eab308",stable:"#eab308",low:"#dc2626",declining:"#dc2626"}[i.demand_trend?.toLowerCase()]||"var(--muted)";e.innerHTML=`
    <div class="guidefyi-card-header">
      <h3 class="guidefyi-card-title">
        <a href="${a}" target="_blank" rel="noopener">${q} ${i.name}</a>
      </h3>
      <p class="guidefyi-card-subtitle">${i.category}</p>
    </div>
    <div class="guidefyi-card-body">
      ...
      ${i.description?`<p class="guidefyi-card-desc">${m(i.description,180)}</p>`:""}
      <div class="guidefyi-card-footer">
        ${u(a,"Full Role Profile")}
      </div>
    </div>
    ${l(t)}
  `}var W={gameDetail:e=>`games/${e}/`,gameList:"games/",publisherList:"publishers/",designerList:"designers/",mechanicList:"mechanics/",categoryList:"categories/",awardList:"awards/",glossaryDetail:e=>`glossary/${e}/`,glossaryList:"glossary/",guideDetail:e=>`guides/${e}/`,guideList:"guides/",faqList:"faqs/",stats:"stats/"},Q={game:e=>`https://boardgamefyi.com/games/${e}/`,designer:e=>`https://boardgamefyi.com/designers/${e}/`,mechanic:e=>`https://boardgamefyi.com/mechanics/${e}/`,publisher:e=>`https://boardgamefyi.com/publishers/${e}/`,glossary:e=>`https://boardgamefyi.com/glossary/${e}/`,guide:e=>`https://boardgamefyi.com/guide/${e}/`,home:"https://boardgamefyi.com/"};async function O(e,r,t){let i=await c(t.apiBase,W.gameDetail(r)),a=Q.game(i.slug),o=i.min_players&&i.max_players?i.min_players===i.max_players?`${i.min_players}`:`${i.min_players}\u2013${i.max_players}`:"\u2014",n=i.playing_time_min&&i.playing_time_max?i.playing_time_min===i.playing_time_max?`${i.playing_time_min}m`:`${i.playing_time_min}\u2013${i.playing_time_max}m`:"\u2014",d=i.bgg_rating!=null?i.bgg_rating.toFixed(1):"\u2014",g=i.bgg_weight!=null?i.bgg_weight.toFixed(1)+"/5":"\u2014";e.innerHTML=`
    <div class="guidefyi-card-header">
      <h3 class="guidefyi-card-title">
        <a href="${a}" target="_blank" rel="noopener">${I} ${i.name}</a>
      </h3>
      <p class="guidefyi-card-subtitle">${i.year_published||""} ${i.bgg_rank?"\xB7 BGG #"+j(i.bgg_rank):""}</p>
    </div>
    <div class="guidefyi-card-body">
      <div class="guidefyi-props">
        <div class="guidefyi-prop">
          <span class="guidefyi-prop-label">Rating</span>
          <span class="guidefyi-prop-value"><span class="guidefyi-rating-badge">${B} ${d}</span></span>
        </div>
        <div class="guidefyi-prop">
        ...
        <div class="guidefyi-prop">
          ...
          <span class="guidefyi-prop-value">${g}</span>
        </div>
      </div>
      ${i.description?`<p class="guidefyi-card-desc">${m(i.description,160)}</p>`:""}
      <div class="guidefyi-card-footer">
        ${u(a,"Full Game Profile")}
      </div>
    </div>
    ${l(t)}
  `}async function U(e,r,t){let i=r.dataset.slug;if(!i){s(e,"Missing data-slug attribute",t);return}f(e);try{let a=r.dataset.entityType||"";switch(t.site){case"calcfyi":a==="formula"?await k(e,i,t):await R(e,i,t);break;case"salaryfyi":a==="role"?await N(e,i,t):await G(e,i,t);break;case"boardgamefyi":await O(e,i,t);break;default:s(e,`Unknown site: ${t.site}`,t)}}catch(a){s(e,a.message,t)}}var _=new Set;function V(e,r,t,i){let a=`defined-term:${t}`;if(_.has(a))return;_.add(a);let o={"@context":"https://schema.org","@type":"DefinedTerm",name:e,description:r.slice(0,300),url:t,inDefinedTermSet:{"@type":"DefinedTermSet",name:`${i} Glossary`,url:t.replace(/glossary\/.*/,"glossary/")}};J(o)}function Y(e,r){let t=`faq:${r}`;if(_.has(t))return;_.add(t);let i={"@context":"https://schema.org","@type":"FAQPage",mainEntity:e.slice(0,10).map(a=>({"@type":"Question",name:a.question,acceptedAnswer:{"@type":"Answer",text:a.answer.slice(0,500)}}))};J(i)}function J(e){let r=document.createElement("script");r.type="application/ld+json",r.textContent=JSON.stringify(e),document.head.appendChild(r)}function le(e,r){return`glossary/${r}/`}function ce(e,r,t){return`https://${r}/glossary/${t}/`}async function K(e,r,t){let i=r.dataset.slug;if(!i){s(e,"Missing data-slug",t);return}f(e);try{let a=await c(t.apiBase,le(t.site,i)),o=a.term||a.name||i,n=ce(t.site,t.domain,a.slug);r.dataset.noSnippet!=="true"&&V(o,a.definition,n,t.name),e.innerHTML=`
      <div class="guidefyi-card-body">
        <div class="guidefyi-glossary-term">${o}</div>
        ...
        <div style="margin-top:10px">${u(n,"Read more")}</div>
      </div>
      ${l(t)}
    `}catch(a){s(e,a.message,t)}}function ge(e,r){return`https://${e}/guide/${r}/`}async function X(e,r,t){let i=r.dataset.slug;if(!i){s(e,"Missing data-slug",t);return}f(e);try{let a=await c(t.apiBase,`guides/${i}/`),o=ge(t.domain,a.slug);e.innerHTML=`
      <div class="guidefyi-card-header">
        <h3 class="guidefyi-card-title">
          <a href="${o}" target="_blank" rel="noopener">${a.title}</a>
        </h3>
        ${a.guide_type||a.category?`<p class="guidefyi-card-subtitle">${a.guide_type||a.category||""}</p>`:""}
      </div>
      <div class="guidefyi-card-body">
        ${a.excerpt?`<p class="guidefyi-card-desc">${m(a.excerpt,200)}</p>`:""}
        ...
        <div style="margin-top:10px">${u(o,"Read Guide")}</div>
      </div>
      ${l(t)}
    `}catch(a){s(e,a.message,t)}}var pe='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>';async function Z(e,r,t){f(e);try{let i=r.dataset.slug,a={};i&&(a.category=i);let o=await A(t.apiBase,"faqs/",a);if(!o.length){s(e,"No FAQs found",t);return}let n=o.slice(0,10);r.dataset.noSnippet!=="true"&&Y(n,`https://${t.domain}/`),e.innerHTML=`
      <div class="guidefyi-card-header">
        <h3 class="guidefyi-card-title">Frequently Asked Questions</h3>
        <p class="guidefyi-card-subtitle">${t.name}</p>
      </div>
      <div class="guidefyi-faq-list">
        ${n.map((d,g)=>`
          <div class="guidefyi-faq-item">
            <div class="guidefyi-faq-q" data-idx="${g}">
              <span>${d.question}</span>
              <span class="guidefyi-faq-arrow">${pe}</span>
            </div>
            <div class="guidefyi-faq-a" data-idx="${g}">${d.answer}</div>
          </div>
        `).join("")}
      </div>
      ${l(t)}
    `,e.querySelectorAll(".guidefyi-faq-q").forEach(d=>{d.addEventListener("click",()=>{let g=d.dataset.idx,y=e.querySelector(`.guidefyi-faq-a[data-idx="${g}"]`),x=d.querySelector(".guidefyi-faq-arrow"),v=y.classList.contains("open");y.classList.toggle("open"),x.classList.toggle("open")})})}catch(i){s(e,i.message,t)}}function ee(e,r,t){let i=r.dataset.placeholder||`Search ${t.entityName}...`,a=`https://${t.domain}${t.searchPath}`;e.innerHTML=`
    ...
    ${l(t)}
  `}function ie(e,r,t){switch(e){case"calcfyi":return`formulas/${r}/`;case"salaryfyi":return t==="role"?`roles/${r}/`:`countries/${r}/`;case"boardgamefyi":return`games/${r}/`;default:return`${r}/`}}function ue(e,r,t){switch(e){case"salaryfyi":return[["Median Salary",b(r.median_salary_usd),b(t.median_salary_usd)],["Top Tax Rate",(r.top_income_tax_rate??"\u2014")+"%",(t.top_income_tax_rate??"\u2014")+"%"],["Cost of Living",String(r.cost_of_living_index??"\u2014"),String(t.cost_of_living_index??"\u2014")]];case"boardgamefyi":return[["BGG Rating",String(r.bgg_rating?.toFixed(1)??"\u2014"),String(t.bgg_rating?.toFixed(1)??"\uFFFD\uFFFD\uFFFD")],["Weight",String(r.bgg_weight?.toFixed(1)??"\u2014"),String(t.bgg_weight?.toFixed(1)??"\u2014")],["Players",`${r.min_players}\u2013${r.max_players}`,`${t.min_players}\u2013${t.max_players}`],["Time",`${r.playing_time_min}\u2013${r.playing_time_max}m`,`${t.playing_time_min}\u2013${t.playing_time_max}m`]];default:return[]}}async function re(e,r,t){let i=r.dataset.slugs;if(!i){s(e,"Missing data-slugs (comma-separated)",t);return}let[a,o]=i.split(",").map(n=>n.trim());if(!a||!o){s(e,"Need two slugs separated by comma",t);return}f(e);try{let n=r.dataset.entityType||"",[d,g]=await Promise.all([c(t.apiBase,ie(t.site,a,n)),c(t.apiBase,ie(t.site,o,n))]),y=ue(t.site,d,g);e.innerHTML=`
      <div class="guidefyi-card-header">
        <h3 class="guidefyi-card-title">${d.name} vs ${g.name}</h3>
        <p class="guidefyi-card-subtitle">${t.name} Comparison</p>
      </div>
        ...
        <div class="guidefyi-compare-col">
          ...
          ${y.map(([x,v])=>`
            <div class="guidefyi-compare-row">
              <span class="guidefyi-compare-label">${x}</span>
              <span class="guidefyi-compare-val">${v}</span>
            </div>
          `).join("")}
        </div>
        <div class="guidefyi-compare-col">
          <div class="guidefyi-compare-name">${g.name}</div>
          ${y.map(([,,x])=>`
            <div class="guidefyi-compare-row">
      ...
      ${l(t)}
    `}catch(n){s(e,n.message,t)}}async function te(e,r){let t=w(e,r),i=$(t,e);i.style.maxWidth="fit-content",i.style.display="inline-flex",i.style.border="none",i.style.background="transparent";let a=e.dataset.slug;if(!a){s(i,"Missing data-slug",r);return}try{let o=await c(r.apiBase,h.formulaDetail(a)),d=(o.formula_latex||o.title).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");i.innerHTML=`
      <span class="guidefyi-badge" style="font-family:ui-monospace,monospace;font-size:11px;letter-spacing:0">
        ${d}
      </span>
    `}catch(o){s(i,o.message,r)}}var p={site:"calcfyi",name:"CalcFYI",domain:"calcfyi.com",accent:"#E11D48",attribute:"data-calcfyi",apiBase:"https://calcfyi.com/api/v1/",searchPath:"/search/",entityName:"Calculators",entitySlug:"compound-interest"};async function fe(e){let r=e.dataset[p.attribute.replace("data-","")]||"entity",t=e.dataset.slug||"";switch(r){case"formula-badge":await te(e,p);return}let i=w(e,p),a=$(i,e);try{switch(r){case"entity":await U(a,e,p);break;case"glossary":await K(a,e,p);break;case"guide":await X(a,e,p);break;case"faq":await Z(a,e,p);break;case"search":ee(a,e,p);break;case"compare":await re(a,e,p);break;case"formula-card":await k(a,t,p);break;default:s(a,`Unknown widget type: ${r}`,p)}}catch(o){s(a,o.message,p)}}var M=new IntersectionObserver(e=>{e.forEach(r=>{r.isIntersecting&&(M.unobserve(r.target),fe(r.target))})},{rootMargin:"200px"});function E(){document.querySelectorAll("[data-calcfyi]").forEach(e=>{e.shadowRoot||M.observe(e)}),document.querySelectorAll("calcfyi-entity, calcfyi-glossary, calcfyi-search, calcfyi-faq, calcfyi-guide, calcfyi-compare").forEach(e=>{if(!e.shadowRoot){let r=e.dataset;r[p.attribute.replace("data-","")]=e.tagName.split("-").pop().toLowerCase(),e.getAttribute("slug")&&(r.slug=e.getAttribute("slug")),e.getAttribute("theme")&&(r.theme=e.getAttribute("theme")),e.getAttribute("style-variant")&&(r.styleVariant=e.getAttribute("style-variant")),e.getAttribute("size")&&(r.size=e.getAttribute("size")),M.observe(e)}})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",E):E();new MutationObserver(()=>E()).observe(document.body,{childList:!0,subtree:!0});
